...
async function load(){
 let x=await api("/api/files?folder="+(state.folder||""));
 let c=$("#content");
...
async function uploadFiles(fs){
 for(const f of fs){
   ...
   let x=await api("/api/files/upload",{method:"POST",body:fd});
...
async function newFolder(){
 ...
 if(n)await api("/api/folders",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n,parent:state.folder})});
 load()
}
...
async function renameFolder(id,n){
 let x=prompt("New folder name",n);
 if(x)await api("/api/folders/"+id,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:x})});
 load()
}
...
async function renameFile(id,n){
 let x=prompt("New filename",n);
 if(x)await api("/api/files/"+id,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:x})});
 load()
}
...
function download(id){
 location="/api/files/"+id+"/download"
}

async function preview(id){
 let r=await api("/api/files/"+id+"/preview");
 let mime=r;
 let w=window.open();
 w.document.write(`<title>Preview</title><iframe style="position:fixed;inset:0;width:100%;height:100%;border:0" src="/api/files/${id}/preview"></iframe>`)
}

async function share(id){
 let x=await api("/api/files/"+id+"/share",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({permission:"viewer",publicLink:true})});
 navigator.clipboard?.writeText(x.data.link);
...
async function search(q){
 ...
 let x=await api("/api/search?q="+encodeURIComponent(q));
...
async function loadRecent(){
 let x=await api("/api/activity");
 $("#content").innerHTML=`<h2>Recent activity</h2><div class="panel">${x.data.activity.map(a=>`<p>• ${esc(a.action)} ${esc(a.file?.name||"")} <span class="muted">${new Date(a.timestamp).toLocaleString()}</span></p>`).join("")||"No recent activity."}</div>`
}

async function loadStarred(){
 let x=await api("/api/files?starred=true");
...
async function emptyTrash(){
 if(confirm("Permanently delete trash?")){
...
async function showStorage(){
 let x=await api("/api/storage"),d=x.data;
 $("#content").innerHTML=`<h2>Storage</h2><div class="panel"><p>${size(d.used)} / ${size(d.quota)} · ${d.percentage}% used</p><div class="bar"><i style="width:${d.percentage}%"></i></div><h3>Largest files</h3>${d.largest.map(f=>`<p>${esc(f.name)} — ${size(f.size)}</p>`).join("")}</div>`
}

async function showAdmin(){
 let x=await api("/api/admin/stats"),d=x.data;
